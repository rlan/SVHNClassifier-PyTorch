...
def _infer(path_to_checkpoint_file, path_to_eval_lmdb_dir):
    model = Model()
    model.load(path_to_checkpoint_file)
    # gpu
    #model.cuda()
    accuracy = Inferer(path_to_eval_lmdb_dir).infer(model)
...
def main(args):
    ...
    _infer(path_to_checkpoint_file, path_to_test_lmdb_dir)
    print 'Done'
